describe('DL: types detectors', function () {
    var object = {},
        array = [],
        fn = function () {},
        string = '',
    ...
    function testType (typeName) {
        describe(typeName + ' detect', function () {
            it ('object is ' + (typeName === 'Object' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](object)).toBe(typeName === 'Object');
                }
            );
            it ('array is ' + ((typeName === 'Array') || (typeName === 'Collection') ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](array)).toBe((typeName === 'Array') || (typeName === 'Collection'));
                }
            );
            it ('fn is ' + (typeName === 'Function' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](fn)).toBe(typeName === 'Function');
                }
            );
            it ('string is ' + (typeName === 'String' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](string)).toBe(typeName === 'String');
                }
            );
            it ('number is ' + (typeName === 'Number' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](number)).toBe(typeName === 'Number');
                }
            );
            it ('boolean is ' + (typeName === 'Boolean' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](boolean)).toBe(typeName === 'Boolean');
                }
            );
            ...
            it ('arguments is ' + ((typeName === 'Arguments') || (typeName === 'Collection') ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](_arguments)).toBe((typeName === 'Arguments') || (typeName === 'Collection'));
                }
            );
            it ('date is ' + (typeName === 'Date' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](date)).toBe(typeName === 'Date');
                }
            );
            it ('regExp is ' + (typeName === 'RegExp' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](regExp)).toBe(typeName === 'RegExp');
                }
            );
            it ('error is ' + (typeName === 'Error' ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](error)).toBe(typeName === 'Error');
                }
            );

            it ('nodeList is ' + ((typeName === 'NodesCollection') || (typeName === 'Collection') ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](nodesList)).toBe((typeName === 'NodesCollection') || (typeName === 'Collection'));
                }
            );

            it ('htmlCollection is ' + ((typeName === 'NodesCollection') || (typeName === 'Collection') ? '' : 'not ') + typeName,
                function () {
                    expect(DL['is' + typeName](htmlCollection)).toBe((typeName === 'NodesCollection') || (typeName === 'Collection'));
                }
            );
